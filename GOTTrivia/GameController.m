...
-(id)init:(NSString*)theDifficulty numberOfQuestions:(NSInteger)numberOfQuestions {
  ...
  if (self) {
    ...
    if ([difficulty  isEqual: @"Easy"]) {
      ...
      path = [[NSBundle mainBundle] pathForResource:
              @"EasyQuestions" ofType:@"plist"];
    }
    else if ([difficulty isEqual:@"Medium"]) {
      path = [[NSBundle mainBundle] pathForResource:
              @"MediumQuestions" ofType:@"plist"];
    }
    else if ([difficulty isEqual:@"Hard"]) {
      path = [[NSBundle mainBundle] pathForResource:
              @"HardQuestions" ofType:@"plist"];
    }
    else {
      // impossible difficulty
      path = [[NSBundle mainBundle] pathForResource:
              @"ImpossibleQuestions" ofType:@"plist"];
      
    }
    NSMutableArray *questionsArray = [[NSMutableArray alloc] initWithContentsOfFile:path];
